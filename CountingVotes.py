...
def XYSwingCount(dataDict ):
    ...
    for node in dataDict:
        #check for NodeIds
        lastDigit = node%10
        if (lastDigit == 0) or (lastDigit == 1) or (lastDigit == 2) or (lastDigit == 3):
            totalX = totalX + 1
    ...
    #print(totalY)
    #print(dictForSwing)
    swingPredX, swingPredY = thoughtProcess(dictForSwing)
    X = swingPredX + totalX
    Y = swingPredY + totalY
...
predDict = dict()
predDict1 = dict()
def thoughtProcess(dictForSwing):
